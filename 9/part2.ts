...
let ropePos = Array.apply(null, Array(10)).map(() => {return {x: 0, y: 0}})
...
for (let i = 0; i < lines.length; i++) {
    ...
    while(stepsRemaining > 0) {
        ropePos[0] = updateHead(ropePos[0], direction);
        for (let knot = 1; knot < ropePos.length; knot++) {
            const oldKnotPos = ropePos[knot];
            const newKnotPos = updateTail(ropePos[knot-1], oldKnotPos);
            if (knot === (ropePos.length - 1)) {
                visited.push({x: newKnotPos.x, y: newKnotPos.y})
            } 
            ropePos[knot] = newKnotPos
        }
