...
class Monkey {
    ...
    takeTurn(monkeys: Monkey[], lcm: number) {
        this.items.forEach((item) => {
            const newValue = this.operation(item);
            const boardValue = newValue % lcm;
            if (newValue % this.test === 0) {
...
const lcm = monkeys.map((x) => x.test).reduce(getLCM);
const rounds = 10_000;

for(let i = 0; i < rounds; i++) {
    for(let m = 0; m < monkeys.length; m++) {
        let currentMonkey = monkeys[m];
        monkeys = currentMonkey.takeTurn(monkeys, lcm);
    }
}
